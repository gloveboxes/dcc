...
- **This repo** brings it to the Altair 8800 / CP/M as an optimized dcc C89
  program, plus a Z80 assembly inference port.
...
`ATTNC89.C` is a from-scratch optimization for the dcc toolchain: because dcc
has a native 32-bit `long`, the hot fixed-point routines (`vdot`, `mvmul`,
`vtmul`, …) use ordinary arithmetic instead of a function-call long package, so
it is both smaller and considerably faster than the earlier BDS C original it
descends from.
