...
/* Single shared I/O buffer: compute_crc, compute_crc_fb, and find_max_bits
 * are never active simultaneously, so they can all share this one buffer. */
static unsigned char g_iobuf [BUFSIZ];
...
#endif
{
  unsigned char * const rbuf = g_iobuf;
  unsigned char oct;
  int ch;
  crc_t tmp;
  crc_t crc = 0;
  ...
  lim_is_zero = cb_is_zero (lim_bits);
...
#ifdef USE_FREAD
        nread = (long)fread (rbuf, 1, sizeof (g_iobuf), fp);
#else
        while ((long)sizeof (g_iobuf) > nread) {
# ifdef multics
...
# ifdef multics
            if (0 != expected_chars &&
              ...
              if (w != -1 || (0 == feof (fp) && 0 == ferror (fp))) {
                if (nread <= (long)(sizeof (g_iobuf) - 4)) {
...
# endif
      ...
      if (0 < bytes_to_process) {
        ...
        crc = crc_update_buffer (crc, tbl, mask32, rbuf, (unsigned int)bytes_to_process);
      }
      ...
      if (0 != pad || 0 != g_pad_auto) {
        ...
        while (0 <= cb_cmp (& rem_bits, & eight)) {
          ...
          if (0 < chunk) {
            ...
            * actually_padded = 1;
            crc = crc_update_buffer (crc, tbl, mask32, zbuf, (unsigned int)chunk);
          }
        }
...
#ifdef USE_FREAD
  {
    unsigned char * const mbuf = g_iobuf;

    for (;;) {
      const long n = (long)fread (mbuf, 1, sizeof (g_iobuf), fp);
      long use = n;
